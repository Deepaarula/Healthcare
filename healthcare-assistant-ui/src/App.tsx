...
export default function App() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 👇 Auto-select tab based on backend response
      if (data.answer && !data.requirement && !data.test_cases && !data.iso_validation) {
        setActiveTab("answer");
                ...
                {Array.isArray(result.test_cases) &&
                  result.test_cases.map((tc: any) => (
                    ...
                    >
                      ...
                      <button className="small-btn mt-2">✔ Mark as Passed</button>
                    </div>
                  ))}
              </div>
            )}
